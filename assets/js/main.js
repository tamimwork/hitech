...
$(function () {
    const $directorsModal = $('#directorsModalOverlay');

    function openDirectorsModal() {
        $directorsModal.addClass('is-open').attr('aria-hidden', 'false');
        $('body').addClass('modal-open-lock');
    }

    function closeDirectorsModal() {
        $directorsModal.removeClass('is-open').attr('aria-hidden', 'true');
        $('body').removeClass('modal-open-lock');
    }

    // Open Modal
    $(document).on('click', '#aboutViewDerectorMember', function (e) {
        e.preventDefault();
        openDirectorsModal();
    });

    // Close Modal
    $(document).on('click', '#directorsModalClose', closeDirectorsModal);
    
    $(document).on('click', '#directorsModalOverlay', function (e) {
        if (e.target === this) closeDirectorsModal();
    });

    $(document).on('keydown', function (e) {
        if (e.key === 'Escape' && $directorsModal.hasClass('is-open')) {
            closeDirectorsModal();
        }
    });
});
...
$('.product-card__price-chart').on('click', function(e) {
    e.stopPropagation();
});
